...
仓库是 private，`UpdateCheck` 发匿名请求，GitHub 一律返回 404。
**每个用户的「关于」面板都会长期显示「检查失败 · HTTP 404」** —— 这比没有这个功能更糟，
它看起来像 bug。

三选一（需要产品决定）：

1. 仓库转 public；
2. `Info.plist` 的 `PulseUpdateFeed` 指向可匿名访问的 feed；
3. 至少把 404 单独识别成「feed 不可达」，不要报成笼统失败。
