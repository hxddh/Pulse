...
| 主题 | 缺陷 | 0.97 动作 |
| --- | --- | --- |
| Pi 官方 JSONL | 假 fixture；不读 `session_info` / 字符串 content / compaction；72h 丢 JSONL；空 SQLite 挡标题 | 按 pi.dev session-format 取 `/name` 与用户正文 |
...
| harvest waitKind | 一律 `Input` | 审批类 tool/phase → Permission，其余 Input |
...
| ID | 项 | 验收 |
| --- | --- | --- |
| P0-0 | Pi 官方会话 | `--path--/timestamp_uuid.jsonl` + 字符串 content 进主行；`session_info.name` 优先；剥 env 标签留提示；compaction `retainedTail`；过期 JSONL 仍收获；空 SQLite 不挡标题 |
...
| P0-8 | 场景 AJ/AK + 测试 | EXPERIENCE **AJ** 官方 Pi、**AK** Hero Honesty；回归 |
