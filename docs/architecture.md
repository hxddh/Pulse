...
| channel | 判据 | 显示 |
| --- | --- | --- |
| `release` | bundle 版本 == 编译版本 | `Pulse 0.22.0` |
| `dev` | 无 bundle 版本（`swift run`） | `Pulse 0.22.0-dev` |
| `mismatch` | 两者不一致 | `0.22.0≠0.21.1` + 橙色警告 |
...
三个都在 `package.sh` 和 CI 里。加上 `swift test`，这就是全部自动防线。
