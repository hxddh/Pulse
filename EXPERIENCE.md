...
表达的聚合信息。状态词与 Glance 同色。右侧只有刷新与「更多操作」两个紧凑入口。
...
| 运行时证据 | 默认展示 | 禁止 |
| --- | --- | --- |
| 结构化会话 | 任务、项目、最近动作、最后活动、开始时间；有数据再加模型调用 / subagent / 事件数 | 裸字段名、无标签箭头、把最近动作写成正在执行 |
| 可验证缓存 | 缓存中真实存在的标题 / 工作区 / 时间，并明说不是当前执行状态 | 从配置里的任意 `name` 猜任务 |
...
状态芯片在仅进程时写「信息有限」，明确数据质量而不是复述进程存在。
