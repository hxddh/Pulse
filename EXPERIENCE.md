...
```
① Header（状态 + 刷新 + 更多操作）
② 分组表头 + Agent 行（默认最多 8 行，每 Agent 模型容量 32 个会话）
③ [另有 N 个…] / [另有 N 个会话未显示]
```

采集器每 Agent 最多读取 64 条，Swift 模型保留其中 32 条，面板默认只展开全局前 8 条。
三层预算分别服务于输入完整性、内存安全与扫视密度；第 33–64 条必须精确计数，
不得让“看起来只显示 8 条”反向限制实际检测。
...
支持健康度默认先展示本机已有有效证据（需要处理、受限或健康），以保证首屏可行动；
「全部」筛选仍保留完整覆盖名单。深度应用数据关闭时，受保护来源必须明确标为隐私受限，
不能让用户把“未读取”误认为“不支持”。
...
| E | 同一 Agent 40 个会话 | 模型保留 32 行并报告「另有 8 个会话未显示」；面板全局 8 行后可展开 |
