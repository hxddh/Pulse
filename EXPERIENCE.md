...
- **次行右端** = 会话时长，例如 `会话 42 分`，用于区分刚开始与长任务
- **观测行**（有数据才出现）= `↑12k ↓3k · sub 2↑/5 · 641 条记录`
...
- 面板用**系统材质**（`.thickMaterial`）。**分组表头不固定（不 pin），因此不需要背景。**
...
- **不要在圆角半透明窗口里画一块不透明的直角矩形。** 窗口比面板高出的那一截会变成第二个表面，
  面板会读成一个贴上去的盒子。用材质，接缝就不存在，而不是去追它。
...
5. **家目录不是项目。** 无法定位的会话归入「无项目」，不发明名字。
   **但「无项目」这个桶不发表头**——用一整行说「下面这两行缺东西」，
   是全面板信息密度最低的一行：一个否定事实，说的是用户已经看得见的行，而且无从处理。
...
- Focus 分级诚实：有 TTY 才聚焦终端页；Warp 下只激活 Warp；只有 cwd 只提供「打开目录」；
