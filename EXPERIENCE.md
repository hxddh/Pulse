...
描述的是当前实现（2.6.0），不是路线图。历史沿革看 [`CHANGELOG.md`](CHANGELOG.md)。
...
| AW | Effect（成效） | **第三类证据：盘上到底变了什么** —— 前两类（agent 自己写下的东西、进程还活着没有）都在回答「它在干什么」，这一类第一次回答**「它干成了什么」**；取值是三条只读命令（`rev-parse --show-toplevel` / `status --porcelain=v1` / `diff --shortstat HEAD`），每条都带 `GIT_OPTIONAL_LOCKS=0` 环境变量**加** `--no-optional-locks`（两者在实践中不等价：真机测试第一跑就抓到 `git diff` 无视 flag 改写 index，flag 只盖住 `status`，环境变量才两个都盖 —— 缺了它「测量」就在写 index 的 stat 缓存并和用户自己的 git 抢 `index.lock`，这正是只读保证必须对真仓库断言而非注释承诺的原因）；**只取计数，不取内容** —— 不要 diff 正文、不要文件路径、不显示分支名；**未知是 -1 不是 0**（不是仓库 / 超时 / 命令失败 / `cwdBestEffort` 未确认，一律不知道），「测了没动」与「没测」是两个答案；**零 adapter** —— 工作副本不属于任何厂商，一份实现覆盖全部 32 个 agent 及以后新增的；省电按 harvest 节奏跳拍、同一工作副本一拍只测一次、单次太慢即进退避并如实报未知；live 且在动（CPU 忙或笔录在长）而**测到**盘上没动 → 叙事说「在动，盘上还没有东西落地」，**不改灯色**；同一仓库根上 ≥2 个 live 本机行 → 行上点名（远端行不参与，它的路径属于另一台机器）—— 这是**只有 Pulse 能看见的事实**，每个 agent 只知道自己
