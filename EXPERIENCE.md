...
- 面板画在**不透明**的 `windowBackgroundColor` 上。放弃毛玻璃，换取强调色文字在任何机器上都可读。
- **分组表头用同一个表面**，不叠材质。它必须不透明（行会从它下面滚过去），但不能是另一个明度——
  `.thickMaterial` 曾让表头成为整个面板最亮的一块，而它承载的是最不重要的信息。
- 面板要撑满自己的窗口。窗口比内容高出一截时，露出来的窗口底色读起来就是第二个表面。
