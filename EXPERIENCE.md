...
描述的是当前实现（0.48.0），不是路线图。历史沿革看 [`CHANGELOG.md`](CHANGELOG.md)。
...
```
① Header（状态 + 刷新 + 更多操作）
② 分组表头 + Agent 行（默认最多 12 行，每 Agent 模型容量 128 个会话）
③ [另有 N 个…] / [另有 N 个会话未显示]
```

采集器每 Agent 最多读取 256 条，Swift 模型保留其中 128 条，面板默认只展开全局前 12 条。
三层预算分别服务于输入完整性、内存安全与扫视密度；第 129–256 条必须精确计数，
...
- **身份行** = 6px 状态灯（等待红 / 运行绿 / 最近灰 / 异常橙）+ Agent 产品名 + 运行时证据等级（仅缓存 / 仅进程时出现）+ 非常态芯片。
  33 个图标不是识别测试；产品名必须可直接扫读。更多操作只在悬停 / 键盘选中时显形，
  但 VoiceOver 动作始终存在
...
| E | 同一 Agent 140 个会话 | 模型保留 128 行并报告「另有 12 个会话未显示」；面板全局 12 行后可展开 |
