...
描述的是当前实现（0.54.0），不是路线图。历史沿革看 [`CHANGELOG.md`](CHANGELOG.md)。
...
7. **关于** —— 版本 · 构建行（`sha · 日期`，可选中；无指纹时显示「开发构建」）·
   ...
   **就地安装按钮仅 notarized stable 出现**。「已是最新」按通道解释（preview 相对
   prerelease 列表；stable 不含 prerelease / 公证前能力在 prerelease）。
