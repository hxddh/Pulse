...
struct ScanMemory {
    ...
    var dashPaths: [String: (path: String, verified: Bool)] = [:]
    /// `lsof` cwd answers per pid, including negative ones.
    var cwd: [Int: (path: String, observedAt: TimeInterval)] = [:]
    /// No `lsof` before this instant after a denied or empty lookup.
    var cwdLookupBackoffUntil: TimeInterval = 0
    /// Last accumulated-CPU reading per pid.
    var cpuSamples: [Int: (cpuSeconds: Double, atMs: Int64)] = [:]
    /// Latched once if `ps` rejects `cputime`/`rss`.
    var psRejectsCPUFields = false
}

enum ScanEngine {
    static let memory = Guarded(ScanMemory())
}
