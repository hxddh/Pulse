...
enum L10n {
    static func t(_ key: Key, _ lang: ResolvedLanguage) -> String {
        ...
        case (.quietHoursHint, .en): return "Waiting notifications still fire. Equal start/end disables quiet hours. Hours wrap past midnight."
        case (.quietHoursHint, .zh): return "Waiting 通知仍会发送。起止相同时安静时段不生效。时段可跨午夜。"
        case (.quietStart, .en): return "From hour"
        case (.quietStart, .zh): return "开始时"
        case (.quietEnd, .en): return "Until hour"
        case (.quietEnd, .zh): return "结束时"
        ...
        case (.hotkeyHint, .en): return "⌘⇧P — reveal Pulse tray · tap notifications to focus waiting agent"
        case (.hotkeyHint, .zh): return "⌘⇧P — 唤出 Pulse 面板 · 点击通知聚焦等待中的 Agent"
        ...
        case (.a11yHint, .en): return "If ⌘⇧P does nothing, grant Accessibility to Pulse in System Settings."
        case (.a11yHint, .zh): return "若 ⌘⇧P 无响应，请在系统设置中为 Pulse 开启辅助功能权限。"
