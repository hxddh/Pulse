...
/// How a row says what it is doing — as a value.
///
...
///
/// `EXPERIENCE.md` §4 is the specification for everything in this type: the
...
struct RowNarrator {
    ...
    func rowContextLine(_ row: AgentRow, omitPath: Bool = false) -> String {
        ...
        let age = row.sessionAgeSeconds(nowMs: nowMs)
    ...
    private func opaqueObservationStory(_ row: AgentRow) -> String {
        ...
        if row.isProcessOnly, row.processStartedMs > 0 {
            ...
            if !ago.isEmpty {
                ...
                let ageSec = max(
                    0,
                    Double(nowMs - row.quality.freshnessMs) / 1000.0
                )
                if ageSec >= 0 {
                    bits.append(String(
