...
笔录里躺着结构化的进展数据：Claude 家族每次更新待办清单，整份清单（每项内容、
状态、当前项）都写进笔录；Codex 的 `update_plan` 同理。而采集端此前**主动过滤**
...
Pulse 到今天为止的所有事实，要么是 **agent 自己写下的东西**（笔录、缓存），要么是
...
它在跑（0.x）、它在等你（1.0）、它在算（2.2）、它在产出笔录（2.1） —— 现在是
**动了 7 个文件 · +142 −38**。

「忙」和「有产出」终于分开：CPU 高、笔录在长、**盘上没动**，这是 agent 在原地打转最
...
跃迁的两个问题：笔录里到底有没有「hook 判决被采纳」的记录（2.5 的第三种下场），
...
内部代码路径**，没有任何证据说明它会落进 Pulse 读得到的笔录 —— 当前采集器一个相关
...
等一个长工具调用 —— 笔录一动不动，进程忙得要死。只看文件，这段时间与「卡住了」
长得一模一样。这一版补上那个信号，并把 1.2 review 里还开着的缺陷一次还清。

### 它在不在真的算

`ps` 的 `%cpu` 在 macOS 上是**进程生命周期的平均值**，对一个开了三小时的 Agent
几乎没有信息量 —— 拿它回答「此刻在忙」就是撒谎。取值方式与 2.1 的笔录增长速率
**同构**：读累计 CPU 时间（`cputime`），记住上一拍的 (累计值, 墙钟)，两拍相减。
同一次 `ps` fork 里顺带取 `rss`，零额外成本、无新权限、不碰 TCC。

于是三种状态第一次可分：CPU 高 + 笔录在长 = 正在产出；**CPU 高 + 笔录不动 = 在算**
（以前这里显示得像卡住）；CPU ~0 + 笔录不动 = 真的停了。停滞行在进程确实在跑时
改说「笔录不动，进程在跑 —— 它在算」，**不改灯色**（仍不是健康绿，只是给停滞一个解释）。
...
mtime 排序，预算按构造花在最新的笔录上（stat 本来就在付，这是重排不是加活）。
...
同时新增 **笔录增长速率**：区分「在干活」与「杵着」，比 mtime 的二值判断细得多。
...
> 就会**静默丢弃磁盘上全部摘要**并把每份笔录从 0 字节重读。已手写 `init(from:)` 兜住。

### Details 成为真正的证据面

最近动作时间线（`Read → Edit → Bash`）、**整场会话** token（与 facts 里「最近一条
消息」的 token 分开说，不让两个数字打架）、笔录增长、真实会话时长、读取完整度 ——
...
也在走，所以是健康的绿。而窗口更看不到 —— 重复发生在笔录中间那段，那正是 1.1 之前
从来没被读过的部分。
...
**远端行保留自己的叙事**（最后听到 / 失联）—— 远端笔录根本不在这台机器上，谈不上打转。
...
native fixture 墙上那条同源的断言（1.4 MB Claude 笔录必须报 0）也一并反转，并且**换成更强的
