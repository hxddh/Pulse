...
12.0 的第三句发布定义「一次扫描不重绘」过去只对设置窗成立；这一版让它对每个表面成立。

- **没变的扫描不唤醒任何表面。** `@Published` 对每次赋值都广播，不管值变没变；扫描路径过去每一拍
  都重写快照、「显示全部」、刷新标志、采集不完整横幅和 Respond 回执，于是托盘、指挥台、会话卡、
  详情窗在等待期间每两秒整体重算一次。现在扫描只写变了的值；快照只在内容变化、屏上有一分钟以内的
  秒级时间、或分钟级时间到点时才替换。新的计数墙 `ScanQuietTests`：同一世界的第二轮扫描发布数为 0，
  失败时点名是哪个属性。
- **严格并发收尾。** App 侧最后 5 条警告清零（跨队列只在主线程使用的值由 `Unchecked` 明写理由）；
  五个 target 全部开启 warnings-as-errors，12.3 的 ratchet 完成使命后退役。
- **规格只写规格。** `EXPERIENCE.md` 从 94 KB 缩到约 51 KB：77 个验收场景移到
  [`docs/scenarios.md`](docs/scenarios.md)，每行写明钉住它的测试 —— 53 个有具名测试，其余 24 个明写
  「人工 / QA 脚本」，这是覆盖缺口，不是免检；`scenario_map.py` 进 gate，防止表格长回规格里或指向
  不存在的测试。§9 代码落点按 12.3 的五个 target 重写。新增场景 BY「扫描静默」。

用户看得到的变化：托盘打开、有会话在等时更省电；界面本身不变。
...
- **所见即所批（H-1）**：完整请求卡上的「同意 / 拒绝」携带渲染时那条请求的 id 与摘要；
  点击前同一行被更新的请求替换时，拒绝发送并提示「请重看」—— 过去会把后到的请求签名。
- **tmux / screen 不再被冻住（H-2）**：父进程链未经过任何 App 就走到 launchd 时，答案是
  「不知道」而不是「不可见」，在场用户的 agent 直接放行。
- **提交不再让证据过期（H-3）**：代码指纹改为工作树内容（路径 + mode + blob id），不再绑定
  commit；检查 → 提交 → 推送的正常流程保持「通过」。旧指纹一律视为过期，请重跑一次检查。
- **检查输出保留尾部、收干净进程（M-1 / M-3）**：检查在独立进程组中运行，超时或结束时整组
  收割；输出保留最后 64 KiB（失败摘要所在）。退出时正在跑的检查重开后记为「中断」。
- **回合结果不再丢（M-2）**：受管会话的输出与退出经同一线程按序送达；取消只杀自己启动的子进程。
  首回合的续接 id 一到就落盘，崩溃后「回复以续」真的续上。
- **同步通道读文件不再跟随链接（M-4）**：spool、inbox、权限请求一律以 `O_NOFOLLOW|O_NONBLOCK`
  打开并确认是普通文件，symlink 与 FIFO 被跳过，永不超出上限。
- **观测管线（M-5）**：session digest 加锁；单条超过 2 MB 的记录被跨过而不是永久停滞；
  中途超时的 adapter 保留未读到的会话行；轮转游标改为稳定位置。
- **安装器（M-6）**：不再覆盖用户自己的 Codex `notify`（原样保留并说明），首次修改前备份
  `config.toml`，经 symlink 的配置文件写回真实文件并保留权限。
...
第一个的答案:方向对,但全部默认全开会把 8.x 的墙搬回来——顶级产品的
做法是**注意力自适应**(Raycast 列表+常驻详情面板;Conductor 完整但有界的
卡)。第二个的病理:弹窗五版进化,其余表面还停在 6.0——产品像只熨了
前襟的衬衫。Linear 式优雅的本质是 token 纪律**无死角**。

### 自适应深度(11.0-α,场景 BV)

信息默认在场,动作按需展开。`RowDepth.tier` 纯表钉死:显式展开→full;
需要你→minimal+ask 卡(问题即深度,摘要噪音不得稀释);**活跃且不拥挤→
digest**——`SessionBriefCard` 纯信息摘要层:被 hero 截断时的完整原话、
...
折叠、展开、重排共用同一条动效曲线(easeOut 0.16,`PulseTheme.motion`)
——两条曲线在一个产品里读起来是两个产品。

7.0–10.0 建的信息与动作能力零删减;诚实规则、能耗、跨机器计数一字不动。

## 10.0.0 — Composition(构图)

判词:「仍然不优雅,杂乱无章。」复盘承认病理:8.x 治好了「看不到」,
病转成了「看不清」——收起行最多七条近同灰线竖排,每条单看都对,
叠起来眼睛没有任何抓手。我们四个版本都在做加法和分线,从来没做过构图。

对照顶级列表(Raycast 的 List 解剖学、Linear 的克制、Agent View)收敛的
语法:行有固定的小解剖学、右缘是元数据柱、次要信息是一条作文不是一摞清单、
克制即自信。

### 一条合成线(10.0-α,场景 BS,重构本体)

收起行的五条灰线(叙事/次行/信号/观测/工作方式)由纯函数 `rowMetaLine`
合成为**一条**:三槽按价值——此刻(循环 > 秒级动作·目标 > 计划步 > phase,
与叙事行同一诚实门)> 产出(错误 > ±/文件 > 子任务 > 进度)> 方式
(工作方式线首位)——稀疏时补项目名,`RowMetaLineTests` 逐格钉死。
**一条事实都不丢**:五线全景整体撤到展开卡(`SessionPanorama`),
选取与去重规则原样生效——换渲染面,不换认识论。展开卡内同步去重:
chips 并入全景与工作细节,model/tokens/context 只出现一次。

### 右柱(10.0-β,场景 BT)

右缘成为一根柱子:相对时间等宽、弱化、右对齐在身份条尾,芯片其后。
扫视沿两根轴走——左轴读事,右轴读量。VoiceOver 同步读合成后的解剖学。

### 披露纪律(10.0-γ,场景 BU)

三层各归其位:**收起=扫视**(微身份条 + hero + 一条 meta);
**展开=理解与行动**(8.x 建的全部深度原样);**列表内卡只给「此刻需要你」**
——权限请求、Respond、回合死亡(interrupted/failed)恢复;受管 idle 的
回复框撤回展开态——每行常驻回复框是墙,不是收件箱。阻塞照旧免点击、
照旧红灯,超时即拒一字不动。
...
上下文、子 agent 在弹窗里**仍然看不到**。复盘承认 8.0 的修法治标——它把这些
事实路由过观测预算、只把「溢出」搬进工作方式线，而真实行很少溢出预算，于是
这条线常年为空；token 用的还是「最近一次调用」寄存器（真实行常为空，整场
累计另存 Details）；工具的显形依赖 `storyOwnsLastAction` 的**静态所有权声明**
——story 实际输出裁到三段时工具被裁掉，声明依旧成立，次行按声明让位，工具
从每条线上同时消失。条件迷宫没有拆掉，只是换了个入口。

### 直构（8.1，场景 BN 重写）

工作方式线**直接由字段构建，不经预算**：最后工具（人话，带活动 spool 记下的
目标叶子）→ token（**整场寄存器优先**、最近一次调用兜底，一线一个口径）→
工作流 → 模型 → 模式 → 上下文 %。测到即渲染；唯一的竞争是顺序，永远不是
是否存在——`RowValueEngineTests` 按事实逐个钉死。

**一个事实一条线，静态成立**：观测行彻底迁出 token/上下文/模型/模式/工作流
（错误、±行、文件、子任务、CPU、增长率留守）；次行的最近动作槽删除（工具的
家在工作方式线与 story）；工具槽只对「这一拍实际渲染了它」的输出让位，
不再信任静态所有权声明。记录数照旧只在两条线都无实质时垫底。

展开卡的「工作方式」层补上**子 agent**（运行中 m/n 或累计 n，未测到缺席）。

## 8.0.0 — Attention 舱

判词落在同一处并且更具体：「tools token skill 等等这类信息完全观测不到」。
对账后承认病根是结构不是采集——这些字段**全都采到了**：当前工具在叙事行的
去重规则后面、工具分布只进 Details、token 排在观测行末位、skill 排在
standing 第五级、模型与上下文同命。每条规则单看都对，叠加起来把「工作方式」
一整层信息的出现率压到近零。另一个更难听的事实：受管回合阻塞在权限请求上时，
行上亮着绿色「运行中」——全产品最高价值的单条信息穿着最无害的衣服。

对齐顶尖同类（Claude Code Agent View、GitHub Agent Mission Control、Devin、
Codex 任务列表）已收敛的三条范式：收件箱优先、产出进列表、每行是直播。

### 价值引擎（8.0-α，场景 BN，重构本体）

`RowValueEngine.split` 改写观测预算的语义：**预算决定事实住哪条线，永不决定
它是否存在**。观测行的 2.1 构成逐字节不变（全量测试为证）；被预算挤掉的
工作类事实（token、上下文、模型、工作流）搬进新的**工作方式线**，最后工具
（人话、对 hero/story/次行查重后）领头。每个事实恰好出现在一条线上，
出现率不为零——由 `RowValueEngineTests` 钉死。展开卡新增「工作方式」全量层：
工具时间线（Read → Edit → Bash）、原始 skill 名、模型、整场 token、
上下文 %——全部是已采字段，零新采集，未测到照旧缺席。

### 真等待重划（8.0-β，场景 BO）

受管回合阻塞在权限请求上 = **真等待**。信号是 Pulse 自己的 spool 请求文件，
回合可证地悬停在判决上——比 hooks 更硬；照 3.0-β/5.0-γ 的先例重划原则，
「Waiting 只来自可证信号」不破反而完整。等待行走全部既有规矩：红灯、
「需要你」分组与计数、通知（正文 = 被请求的那件事本身：
`Bash: git push --dry-run`，字段序沿厂商权限标题，经 sanitizer）。
受管 `idle` 仍不是 Waiting——你的回合可见（列表内回复框），不报警；
红灯留给阻塞。超时两分钟自动拒绝一字不动。

**收件箱**：ask 卡免点击住进列表——受管权限卡、Respond 卡、受管回复框
以紧凑面直接渲染在行下，与展开卡互斥、永不双渲染。回答不再要任何点击。

### 直播行（8.0-γ，场景 BP）

收起行答得了「它干成了什么」：受管行的成本·回合与本回合落盘 ±行进观测行
（第一手事实）；错误原文一行直进收起行（橙、自述新鲜门）；受管行在列表内
即有回合控制（运行中 = 当前工具 + 终止；结束 = 回复框）；展开卡带受管对话
尾巴（最近 5 步、`↳` 配对、内存流零磁盘读）。观察行的全文仍在指挥台——
每拍磁盘读不是 ambient 成本，边界明说。
